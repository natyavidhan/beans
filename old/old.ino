...
<script>
  ...
  function drawHorizon(rollDeg, pitchDeg) {
    ...
    for (let deg = -90; deg <= 90; deg += 5) {
      if (deg === 0) continue;
      let y = pitchPx - deg * (r / 90.0);
      if (y < -r || y > r) continue;

      let length = (deg % 10 === 0) ? 30 : 15;
      ...
      if (deg % 10 === 0) {
        ...
        ctx.fillText(Math.abs(deg), length + 12, y);
        ctx.fillText(Math.abs(deg), -length - 12, y);
      }
    }
...
void readIMU(float dt) {
  ...
  gyroRollRate  = ((gy / 65.5)) - gyroX_offset;
  gyroPitchRate = (-(gz / 65.5)) - gyroY_offset;
  gyroYawRate   = (gx / 65.5) - gyroZ_offset;
